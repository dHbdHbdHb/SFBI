...
mapview(complete_map_large, zcol = "total")
```
...
complete_map_large_nodriving <- complete_large_nodriving %>% 
  ...
  cbind(readRDS("sf_cbgs.rds") %>% select(GEOID)) %>% 
  st_as_sf() 
mapview(complete_map_large_nodriving, zcol = "total")
```
...
```{r echo=FALSE}
mapview(complete_map_large, zcol = "walking") 
```
...
```{r}
mapview(complete_map_large, zcol = "cycling")
```

Regardless, some interesting conclusions that can be drawn from this analysis is that perhaps there are some low hanging fruits in improving the completeness of S.F. neighborhoods.  Increasing the more residents’ inclination to bike is potentially a cost effective way to build better neighborhoods. 

The final step of this analysis would be to compare to public transportation.  It would be interesting to see how the cost of comprehensive, desirable public transport would compare to access from biking.  This is certainly not to pit the MUNI budget against that of the SFMTA bike planning department; there are still many interesting considerations like biking plus public transit (i.e. putting a bike on the front of a bus) and how could affect completeness.  Additionally it is important to consider that many residents in a city are not able to bike, and investment in public transportation better serves that population.  Also, much of these specific scores are a product of assumptions I made in the value decay formula.  By squaring the factor in the transportation score function, I am producing much higher completeness scores than had I used the same computation from the amenity value decay.
